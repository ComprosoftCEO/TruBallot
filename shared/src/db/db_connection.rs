...
impl FromRequest for DbConnection {
  ...
  type Future = Ready<Result<DbConnection, SharedError>>;
  type Config = ();
...
pub fn get_connection_from_request(req: &HttpRequest) -> Result<DbConnection, PoolError> {
  let pool = req
    .app_data::<web::Data<PgPool>>()
    .expect("PgPool app data should be set");
